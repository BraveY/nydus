...
// Mock exec.Command for testing runNydusImageUnpack
...
func TestEdgeCases(t *testing.T) {
	...
	t.Run("Test createOCIConfig with complex layers", func(t *testing.T) {
		layers := []ocispec.Descriptor{
			{
		...
		config, err := createOCIConfig(nil, layers)
		assert.NoError(t, err)
		assert.Equal(t, 2, len(config.RootFS.DiffIDs))
		assert.Equal(t, 2, len(config.History))
	})
	...
	t.Run("Test extractNydusLayer with symlinks", func(t *testing.T) {
		tmpDir, err := os.MkdirTemp("", "test-extract-*")
		assert.NoError(t, err)
		defer os.RemoveAll(tmpDir)

		// Create tar file with symlink
		tarFile, err := os.CreateTemp(tmpDir, "test-*.tar")
		assert.NoError(t, err)
		...
		assert.NoError(t, err)
		_, err = tarWriter.Write([]byte("bootstrap"))
		assert.NoError(t, err)
		err = tarWriter.Close()
		assert.NoError(t, err)
		err = tarFile.Close()
		...
		bootstrapPath, _, err := extractNydusLayer(tarFileName, outputDir)
		assert.NoError(t, err)
		assert.NotEmpty(t, bootstrapPath)
	})
...
func TestRunNydusImageUnpack(t *testing.T) {
	...
	t.Run("Test successful unpack", func(t *testing.T) {
		// Mock exec.Command to return successful command
		...
		err = runNydusImageUnpack("nydus-image", bootstrapPath, blobPath, outputDir)
		assert.NoError(t, err)
	})

	t.Run("Test command failure", func(t *testing.T) {
		// Mock exec.Command to return failing command
		execCommand = func(name string, arg ...string) *exec.Cmd {
			cs := []string{"-test.run=TestHelperProcess", "--", "nydus-image-fail"}
			cs = append(cs, arg...)
		...
		err = runNydusImageUnpack("nydus-image", bootstrapPath, blobPath, outputDir)
		assert.Error(t, err)
		// Check that we get an exit error from the mock command
		assert.Contains(t, err.Error(), "exit status")
	})

	t.Run("Test with non-existent bootstrap file", func(t *testing.T) {
		execCommand = fakeExecCommand
		...
		bootstrapPath := filepath.Join(tmpDir, "non-existent-bootstrap")
		blobPath := filepath.Join(tmpDir, "blob")
		err = os.WriteFile(blobPath, []byte("blob data"), 0644)
		assert.NoError(t, err)
		...
		// This should still call the command, but may fail due to missing file
		err = runNydusImageUnpack("nydus-image", bootstrapPath, blobPath, outputDir)
		// Since we're using a mock, this should succeed even with non-existent bootstrap
		assert.NoError(t, err)
	})
...
func TestUnpackNydusLayers(t *testing.T) {
	...
	t.Run("Test with only nydus layers", func(t *testing.T) {
		tmpDir, err := os.MkdirTemp("", "test-unpack-*")
		assert.NoError(t, err)
		defer os.RemoveAll(tmpDir)

		// Create nydus layer tar
		nydusLayerFile := filepath.Join(tmpDir, "nydus-layer.tar")
		nydusFile, err := os.Create(nydusLayerFile)
		assert.NoError(t, err)
		tarWriter := tar.NewWriter(nydusFile)
		...
		_, err = unpackNydusLayers(context.Background(), ReverseOpt{NydusImagePath: "nydus-image"}, tmpDir, layers)
		// This will likely fail due to missing nydus-image binary
		assert.Error(t, err)
	})
	...
	t.Run("Test with invalid layer file", func(t *testing.T) {
		tmpDir, err := os.MkdirTemp("", "test-unpack-*")
		assert.NoError(t, err)
		defer os.RemoveAll(tmpDir)
...
func TestUtilityFunctions(t *testing.T) {
	...
	t.Run("Test createOCIConfig with nil manifest", func(t *testing.T) {
		layers := []ocispec.Descriptor{
			{
				Digest:    "sha256:abc123",
				Size:      1024,
		...
		assert.Equal(t, 1, len(config.RootFS.DiffIDs))
		assert.Equal(t, 1, len(config.History))
	})

	t.Run("Test createOCIConfig with empty layers", func(t *testing.T) {
		config, err := createOCIConfig(nil, []ocispec.Descriptor{})
		assert.NoError(t, err)
		assert.NotNil(t, config)
		assert.Equal(t, 0, len(config.RootFS.DiffIDs))
		assert.Equal(t, 0, len(config.History))
	})
